...
export function buildPreviewBridge(parentOrigin: string) {
  ...
  return String.raw`(() => {
    ...
    const validateSource = (requestId, expected) => {
      cancelValidation();
      if (!initialized || typeof requestId !== 'string' || requestId.length !== 36 || expected?.generation !== generation || typeof expected?.anchor?.hash !== 'string' || expected.anchor.hash.length !== 64) return;
      if (expected.text!==undefined && (typeof expected.text!=='string'||expected.text.length>2000))return;
      if (expected.styles!==undefined && (typeof expected.styles?.borderRadius!=='string'||expected.styles.borderRadius.length>120||Object.keys(expected.styles).length!==1))return;
      if (expected.text===undefined && expected.styles===undefined)return;
      ...
      const matches = () => {
        ...
        if ((expected.text || expected.styles) && (element.getClientRects().length===0 || element.checkVisibility && !element.checkVisibility({checkOpacity:true,checkVisibilityCSS:true}))) return false;
        ...
        try {
          ...
          return anchor.file===expected.anchor.file && anchor.hash===expected.anchor.hash && anchor.start===expected.anchor.start && anchor.end===expected.anchor.end && anchor.tag===expected.anchor.tag && (expected.text===undefined||element.textContent===expected.text) && (!expected.styles||getComputedStyle(element).borderRadius===expected.styles.borderRadius);
        } catch { return false; }
      };
      const check=async()=>{
