...
export default function Action(){const [count,setCount]=useState(0);return <section><h1>Start</h1><button id="action" onClick={()=>setCount(count+1)}>Start</button><output>{count}</output></section>}`;
...
function Fixture(){
  ...
  return <main style={{padding:24,fontFamily:'system-ui'}}><h1>Live JSX source audit</h1><p>This is a synthetic Vite test, not Finite import acceptance.</p>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 350px',gap:20}}>
      {url?<PreviewFrame baseUrl={url} route="/" frameId="action" title="Mapped Vite preview" mode={mode} onScroll={()=>{}} inspectCommand={{sequence}} onInspection={value=>{snapshot=value;setInspection(value);}} style={{width:'100%',height:500}}/>:<div/>}
      <LiveInspector tab="design" available={Boolean(url)} inspection={inspection} onRefresh={()=>setSequence(sequence+1)} onSelect={()=>{}} onSource={anchor=>void open(anchor)} onDiscuss={()=>{}}/>
