...
function Fixture() {
  ...
  return <><h1>Actual iframe / bridge regression fixture</h1><button onClick={()=>setPins(true)}>Pin comparisons</button><button onClick={()=>setFocus('frame-0')}>Focus first</button><button onClick={()=>setFocus(null)}>Close focus</button><button onClick={()=>setZoom(.4)}>Zoom out</button><button onClick={()=>setZoom(1)}>Zoom in</button><output id="snapshots">{JSON.stringify(snapshots)}</output><output id="count">{schedule.total}</output><div id="canvas">{schedule.canvas.map(surface)}</div>{schedule.focus&&<div id="focus">{surface(schedule.focus)}</div>}</>;
}
